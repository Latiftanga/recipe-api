...
class PrivateRecipeAPITests(TestCase):
    ...
    def test_create_recipe_with_tags(self):
        """Test creating recipe with assigned tags"""
        tag1 = sample_tag(user=self.user, name='Vegan')
        tag2 = sample_tag(user=self.user, name='Dessert')

        payload = {
            'title': 'Avocado lime cheese',
    ...
    def test_partial_update_recipe(self):
        """Test updating recipe using path"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self):
        """Test returning recipes with specified tag"""
        ...
        recipe3 = sample_recipe(user=self.user, title='Kontomire stew')
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        recipe3 = sample_recipe(user=self.user, title='Steaks and mushrooms')
    ...
    def test_retrieve_ingredients_assigned_to_recipes(self):
        """Test retrieving ingredients assigned to recipes"""
        ingredient1 = Ingredient.objects.create(user=self.user, name='Apples')
        ingredient2 = Ingredient.objects.create(user=self.user, name='Turkey')
        recipe = Recipe.objects.create(
            title='Apples crumbles',
            time_minutes=5,
            price=10,
            user=self.user
        )
